...
function mySlider(ele) {
    ...
    // 애니메이션 이징 변수
    const aniE = "easeOutQuad";

    $(ele).find(".abtn").click(function (e) {
        // console.log("진입:",prot);

        // a요소 때문에 위쪽으로 튀는 현상 방지 위해 막음
        e.preventDefault();
    ...
    console.log("슬라이드 widht: ", sldW);

    // 왼쪽으로 드래그시 튀는 현상 방지 위해 위치 보정값 공유하기
    let spos = 0; ///클릭시엔 0이 필요하다

    // 3.드래그가 끝날 때의 방향 설정하기
    // 처음 left값은 0임
    // 100px 기준으로 방향을 결정한다!
    // 드래그 멈춤 이벤트에서 함수 설정
    slide.on("dragstop", function () {
        // 슬라이드의 left값 (#viewer>ul의 left값 -> 왜? ul을 잡고 움직여야 li들이 전부 다 같이 움직이니까!)
        spos = slide.position().left;
        // 🔥🔥offset()은 전체 화면이 기준임
        // 🔥🔥position()은 부모자격이 있는 박스가 기준임
        // ->>여기서 부모자격이 있는 박스는 #viewer이고, 여기에서부터 위치 이동을 하는 것이라서 position()을 쓰는 것임
        console.log("슬라이드 left: ", spos);
        ...
        // (1)왼쪽방향으로 끌 때 = 오른쪽 버튼을 클릭한 것과 동일한 효과
        if (spos < -50) $(ele).find(".rb").trigger("click");
        // (2)오른쪽방향으로 끌 때 = 왼쪽 버튼을 클릭한 것과 동일한 효과
        else if (spos > 50) $(ele).find(".lb").trigger("click");
        // (3)기타 제자리로!
        else slide.animate({ left: "0" }, 300, aniE);
    }); /////////////////// dragstop 이벤트 ////////////////////////
}
