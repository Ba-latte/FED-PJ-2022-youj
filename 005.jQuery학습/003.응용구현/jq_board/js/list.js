...
function bindList(pgnum){ // pgnum - 페이지번호
    ...
    for (let i = (pgnum - 1) * pgblock; i < pgnum * pgblock; i++) {
        ...
        // 존재하는 데이터까지만 바인딩하기
        if(i < totnum){
            // 마지막번호가 length보다 1 작으니까 이런 조건이 가능! -> 돌기는 돌아고 코드 만드는 이 안으로 진입 못한대
            // length는 for문 들어오면 계속 반복해서 찾아야하니까.. 이런건 약간 비효율적! 그래서 for문 들어오기 전에 미리 변수에 담아두고 쓰는 게 좋음
            blist += `
                <tr>
                    <td>${bdata[i]["idx"]}</td>
                    <td>
