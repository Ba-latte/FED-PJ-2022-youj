    ...
    ***********************************/
    ...
    .click(function(){
        let fn = () => {// 콜백함수
            ...
            msg.html("여긴 없겠지??")
            .fadeIn(200)
            .delay(1000) // 1초 지연(애니 앞에서만 적용됨)
            .fadeIn(200, ()=>{
                // 두번째 메시지
                msg.html(`그래도 무서우니 <br> 위층으로 가자!`);
                
                // 다음버튼 보이기
                $(this).next().delay(500).slideDown(300);
            }) // fadeIn : 딜레이 적용 위해 똑같이 작성함 (딜레이 뒤에 css 있으면 안돼; 소용 없음!)
        }; ///////////// fn함수 /////////
    ...
    .click(function(){
        let fn = () => {// 콜백함수
            ...
            .fadeIn(200, ()=>{
                ...
                // 달려들기
                .animate({
                    right: (bd.eq(7).width()*1.2) + "px" // *1.2 : 자기 크기의 1.2배정도,,
                }, 1000, "easeOutBounce", ()=>{
                    // function(){}을 쓰면 this가 '좀비'가 되어버리니까! 화살표함수로 써야 this가 밖으로 나가서 '버튼'이 된다!
    ...
    .click(function(){
        let fn = () => {// 콜백함수
            // 주사기 돌리기 (animate는 transform 적용 안되니 주의할 것!!!)
            $(".inj").css({
                transform:"rotate(-150deg)", // 반시계방향 회전
                transition: ".5s .5s", //0.5초 후에 0.5초간 실행됨
                zIndex : "99999" // z-index 높이기 (미니언즈보다 위에 위치)
            }); ////////// css //////////////
    ...
    .click(function(){
        let fn = () => {// 콜백함수
            ...
            // 메시지 보이기
            msg.html(`어서 위층으로 가자!!`).fadeIn(300);
    ...
    .click(function(){
        let fn = () => {// 콜백함수
            ...
            function(){ // 콜백함수
                // 헬기 이동 완료 후 헬기 이미지 변경 (여기서 this는 .heli임)
            ...
            .animate({
                ...
                tit.addClass("on");
                // 2단계 : 맨 아래까지 떨어짐
                // -> 3초후 간판에 class "on2" 추가하기 (1단계 실행 중에도 3초 세는 것임)
                setTimeout(() => {
                    tit.addClass("on2"); // 기존 클래스에 추가로 더해줌
                }, 3000);
                ...
                // 추가 구현 : 건물이 무너진 후(6초+건물기다리고 무너진시간 8초 = 14초후) 좀비 한마리 기어올라와서 오른쪽으로 이동하기
                setTimeout(() => {
                    // 건물 기울기 원상복구하기
