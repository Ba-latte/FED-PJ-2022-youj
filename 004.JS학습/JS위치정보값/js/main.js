...
window.addEventListener("DOMContentLoaded", () => {
    ...
    // .inbox하위 .bird(벌새) 위에서 마우스 움직일 때 이벤트 버블링으로 offsetX,offsetY는 이벤트 대상의 위치를 리턴하므로 벌새값이 .inbox 이벤트 찍기에 나타난다
    // .bird의 같은 이벤트를 버블링 막기를 하면 벌새위에서는 이벤트가 발생하지 않는다(버블링되지 않는다)
